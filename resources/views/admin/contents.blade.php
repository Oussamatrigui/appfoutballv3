                                    ...
                                    <tbody>
                                        @foreach ($contents as $content)
                                            <tr>
                                                <td>{{ $increment }}</td>
                                                <td>
                                                    <img src="/storage/content_images/{{ $content->content_image }}"
                                                        style="height : 50px; width : 50px" class="img-circle elevation-2"
                                                        alt="User Image">
                                                </td>
                                                <td>{{ $content->titre_article }}</td>
                                                
                                                <td>{{ $content->nom_auteur }}</td>
                                                <td>
                                                    @if ($content->status != 0)
                                                        <a href="{{ url('/desactiver_content/' . $content->id) }}"
                                                            class="btn btn-success">Déactiver</a>
                                                    @else
                                                        <a href="{{ url('/activer_content/' . $content->id) }}"
                                                            class="btn btn-warning">Activer</a>
                                                    @endif
                                                    <a href="{{ url('/edit_content/' . $content->id) }}"
                                                        class="btn btn-primary"><i class="nav-icon fas fa-edit"></i></a>
                                                    <a href="{{ url('/delete_content/' . $content->id) }}" id="delete"
